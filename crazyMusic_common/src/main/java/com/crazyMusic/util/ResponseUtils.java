...
public final class ResponseUtils {
    ...
    public static void rsaObject(Object object){
   		 ...
   		 if(dataMap == null || dataMap.isEmpty()){
   			 ...
   			 for (String key : keySet) {
   				 if((dataMap.get(key)+"") != null && !"".equals(dataMap.get(key)))
   					 dataMap.put(key, SymmetricEncoder.AESEncode(Const.AESKEY, dataMap.get(key)+""));
				 }
   		 	}
    ...
    public static void putRSAJsonResponse(HttpServletResponse response, JSONObject json) {
    	 putTextResponse(response, json.toString());
    	/* if (json == null) {
             putTextResponse(response, "{}");
         }
